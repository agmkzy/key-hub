import {candidates,optimisePath,voicings,pc,mod,noteAt,noteText,scale,performance} from './music-v3.2.js';
...
export function arrangementCandidates(c,s){
 ...
 if(hasMelody){
  shapes=shapes.filter(v=>v.rh.at(-1).midi-v.rh[0].midi<=12).map(v=>{
   ...
   return describeVoice(c,{...v,lh,rh:[],local:Math.abs((lh[0].midi+lh.at(-1).midi)/2-(53+shift))*.12+(s.voicing==='Closed'?Math.max(0,lh.at(-1).midi-lh[0].midi-7)*.3:s.voicing==='Wide'?Math.abs(lh.at(-1).midi-lh[0].midi-12)*.25:0)});
  });
 }else{
  ...
  if(legacy)shapes=candidates(c,{...s,hands:legacy,harmonyHand:s.fullHarmony?'right':undefined});
  shapes=shapes.map(v=>{
...
export function melodyMap(c,key,minor){
 ...
 const colour=c.notes.slice(3);if(!colour.length&&!c.applied&&!c.borrowed&&!c.chromatic.length&&!land.some(n=>pc(n)===pc(seventh)))colour.push(seventh);
