export const MAJOR=['C','D♭','D','E♭','E','F','G♭','G','A♭','A','B♭','B','F♯','C♯','C♭'];
export const MINOR=['C','C♯','D','E♭','E','F','F♯','G','G♯','A','B♭','B','D♯','A♯','A♭'];
...
export const EXTENSIONS=['triad','sus2','sus4','7','maj7','9','maj9','add9','dim7'];
export const ROOT_GROUPS=[['C'],['C♯','D♭'],['D'],['D♯','E♭'],['E'],['F'],['F♯','G♭'],['G'],['G♯','A♭'],['A'],['A♯','B♭'],['B']];
export const CHORD_QUALITIES=[
 {id:'major',label:'Major',group:'Triads',quality:'major',ext:'triad'},
 {id:'minor',label:'Minor',group:'Triads',quality:'minor',ext:'triad'},
 {id:'dim',label:'Diminished',group:'Triads',quality:'dim',ext:'triad'},
 {id:'aug',label:'Augmented',group:'Triads',quality:'aug',ext:'triad'},
 {id:'sus2',label:'sus2',group:'Suspended',quality:'major',ext:'sus2'},
 {id:'sus4',label:'sus4',group:'Suspended',quality:'major',ext:'sus4'},
 {id:'maj7',label:'Major 7',group:'Sevenths',quality:'major',ext:'maj7'},
 {id:'7',label:'Dominant 7',group:'Sevenths',quality:'major',ext:'7'},
 {id:'m7',label:'Minor 7',group:'Sevenths',quality:'minor',ext:'7'},
 {id:'mmaj7',label:'Minor-major 7',group:'Sevenths',quality:'minor',ext:'maj7'},
 {id:'m7b5',label:'Half-diminished / m7♭5',group:'Sevenths',quality:'dim',ext:'7'},
 {id:'dim7',label:'Diminished 7',group:'Sevenths',quality:'dim',ext:'dim7'},
 {id:'add9',label:'add9',group:'Ninths / added colour',quality:'major',ext:'add9'},
 {id:'maj9',label:'maj9',group:'Ninths / added colour',quality:'major',ext:'maj9'},
 {id:'9',label:'9',group:'Ninths / added colour',quality:'major',ext:'9'},
 {id:'m9',label:'m9',group:'Ninths / added colour',quality:'minor',ext:'9'}
];
export function rootDegree(root,key){return (letters.indexOf(root[0])-letters.indexOf(key[0])+7)%7;}
export function chordItem(root,qualityId,key){const q=CHORD_QUALITIES.find(q=>q.id===qualityId);if(!q)throw Error('Unsupported chord quality');return {degree:rootDegree(root,key),root,quality:q.quality,ext:q.ext};}
export function transposeChordItem(item,fromKey,toKey){if(!item.root||pc(fromKey)===pc(toKey))return {...item};return {...item,root:spell(toKey,rootDegree(item.root,fromKey),mod(pc(item.root)-pc(fromKey))),degree:rootDegree(item.root,fromKey)};}
export function availableExtensions(c){return c.quality==='aug'?['triad','sus2','sus4']:EXTENSIONS.filter(e=>e!=='dim7'||c.quality==='dim');}

export function chord(key,minor,item,next){
 const s=scale(key,minor),source=item.borrowed?scale(key,!minor):s;
 const degree=item.root?rootDegree(item.root,key):(item.degree??0);
 let root=source[degree],base=[0,2,4].map(n=>source[(degree+n)%7]);
 ...
 if(item.root){root=item.root;quality=item.quality||quality;}
 const ext=item.ext||'triad';if(ext==='dim7')quality='dim';let degrees=[0,2,4],semitones=[0,['major','aug'].includes(quality)?4:3,quality==='dim'?6:quality==='aug'?8:7];let suffix=quality==='minor'?'m':quality==='dim'?'dim':quality==='aug'?'aug':'';
 if(ext==='sus2'||ext==='sus4'){degrees=[0,ext==='sus2'?1:3,4];semitones=[0,ext==='sus2'?2:5,7];suffix=ext;}
 else{
  if(ext==='dim7'){degrees.push(6);semitones.push(9);suffix='dim7';}
 ...
 let roman=(item.borrowed&&!minor&&[2,5,6].includes(degree)?'♭':'')+romans[degree];
 let func=[0,2,5].includes(degree)?'Tonic':[1,3].includes(degree)?'Predominant':'Dominant';
 if(minor&&!item.borrowed&&[4,6].includes(degree))func='Modal tension';
 if(ext!=='triad'){
  if(ext==='dim7')roman=roman.replace('°','').toLowerCase()+'°7';
  else if(ext==='sus2'||ext==='sus4')roman=roman.replace('°','').toUpperCase()+ext;
  else if(quality==='dim'&&(ext==='7'||ext==='9'))roman=roman.replace('°','ø')+ext;
  else roman+=(quality!=='major'&&ext.startsWith('maj')?'('+ext+')':ext);
 }
 if(target){roman='V'+(ext==='triad'?'':ext)+'/'+target.roman;func='Dominant of '+target.root;}
 const result={root,name:root+suffix,quality,ext,notes,degrees,semitones,roman,func,chromatic:notes.filter(n=>!s.some(a=>pc(a)===pc(n))),borrowed:!!item.borrowed,degree,applied:!!item.applied,target:target?.name,item};
 return item.root?contextualise(result,key,minor,next):result;
}
function contextualise(c,key,minor,next){
 const s=scale(key,minor),same=(a,b)=>a.notes.length===b.notes.length&&a.notes.every(n=>b.notes.some(t=>pc(n)===pc(t)));
 const native=Array.from({length:7},(_,degree)=>chord(key,minor,{degree}));
 // Exact diatonic pitch content takes priority over a speculative function.
 for(const triad of native){const matching=chord(key,minor,{degree:triad.degree,ext:c.ext});if(pc(c.root)===pc(matching.root)&&same(c,matching)&&!c.chromatic.length)return {...c,degree:triad.degree,roman:matching.roman,func:matching.func,context:null};}
 if(c.ext==='dim7'&&next){const target=chord(key,minor,next);if(target.quality!=='dim'&&c.root===spell(target.root,6,11)){
   const known=native.find(n=>pc(n.root)===pc(target.root)&&n.quality===target.quality);return {...c,roman:known?'vii°7'+(known.degree===0?'':'/'+known.roman):'Chromatic',func:'Leading-tone approach',context:'Leading-tone diminished approach to '+target.name};
 }}
 if(c.quality==='major'&&['7','9'].includes(c.ext)){
  const destinations=native.filter(t=>t.quality!=='dim'&&c.root===spell(t.root,4,7));
  if(destinations.length===1){const t=destinations[0];return {...c,roman:t.degree===0?'V'+c.ext:'V'+c.ext+'/'+t.roman,func:t.degree===0?'Dominant':'Secondary dominant',context:(t.degree===0?'Dominant of ':'Secondary dominant of ')+t.name,target:t.name};}
 }
 // Parallel-key borrowing requires a matching triad and notes from that parallel scale.
 const parallel=scale(key,!minor);
 for(let degree=0;degree<7;degree++){
  const borrowed=chord(key,minor,{degree,borrowed:true,ext:c.ext});
  if(pc(c.root)===pc(borrowed.root)&&same(c,borrowed)&&c.notes.every(n=>parallel.some(t=>pc(t)===pc(n))))return {...c,degree,roman:borrowed.roman,func:borrowed.func,borrowed:true,context:'Borrowed from '+key+(minor?' major':' minor')};
 }
 return {...c,roman:'Chromatic',func:'Chromatic',context:'Outside '+key+(minor?' minor':' major')};
}
export function progressionChords(key,minor,items){return items.map((item,i)=>chord(key,minor,item,items[i+1]));}

export function diatonicExtension(key,minor,item){if(item.ext==='dim7'||item.quality==='aug')return item.ext||'triad';if(item.applied)return '7';if(item.root){const c=chord(key,minor,item);if(c.quality!=='major'||c.chromatic.length)return '7';}let c=chord(key,minor,{...item,ext:'triad'}),s=scale(key,item.borrowed?!minor:minor);return mod(pc(s[(c.degree+6)%7])-pc(c.root))===11?'maj7':'7';}
export function describe(c){if(c.context)return c.context+'.';if(c.applied)return 'A temporary dominant pulling towards '+c.target+'.';if(c.borrowed)return 'Borrowed colour from the parallel key.';if(c.func==='Tonic')return c.degree===0?'Home. Stable and resolved.':'A softer resting place.';if(c.func==='Predominant')return 'Moves away from home, towards V.';if(c.func==='Modal tension')return 'Gentle pull; no raised leading tone.';return 'Tension that draws you home.';}
...
export function candidates(c,settings={}){
 ...
 for(const inv of inversions){if(inv>=c.notes.length)continue;const bassName=c.notes[inv];
  for(let bass=36+shift; bass<=55+shift; bass++){
   ...
   if(!settings.harmonyHand&&choice!=='0'&&ix.length>=4&&lh.some(n=>pc(n.name)===pc(c.root)))ix=ix.filter(i=>i!==0);
   if(settings.harmonyHand)ix.sort((a,b)=>mod(pc(c.notes[a])-pc(c.root))-mod(pc(c.notes[b])-pc(c.root)));
...
export function connectionSuggestions(key,minor,from,to,settings={}){
 ...
 for(const ext of ['maj7','add9','maj9','7','9'].filter(e=>availableExtensions(current).includes(e))){if(ext===(from.ext||'triad'))continue;const item={...from,ext,lock:undefined,inversion:'auto',beats:2};const c=chord(key,minor,item);const added=c.notes.filter(n=>!current.notes.includes(n));const shared=c.notes.filter(n=>target.notes.some(t=>pc(t)===pc(n)));if(!added.some(n=>target.notes.some(t=>pc(t)===pc(n))))continue;
 ...
 if(target.quality!=='dim'){const item=(to.root||to.applied)?chordItem(spell(target.root,4,7),'7',key):{degree:to.degree,applied:true,appliedTargetBorrowed:!!to.borrowed,ext:'7',inversion:'auto',beats:2};let c=chord(key,minor,item);suggestions.push({kind:'bridge',item,chord:c,shared:c.notes.filter(n=>target.notes.some(t=>pc(t)===pc(n))),reason:'Dominant of '+target.name+'. '+c.notes[1]+' leads up a semitone to '+target.root+'.',score:0});}
 if(target.quality!=='dim'){const item=chordItem(spell(target.root,6,11),'dim7',key),c=chord(key,minor,item,to);suggestions.push({kind:'approach',item,chord:c,shared:c.notes.filter(n=>target.notes.some(t=>pc(t)===pc(n))),reason:'Leading-tone diminished approach: '+c.root+' rises a semitone to '+target.root+'.',score:-1});}
 suggestions=suggestions.filter(s=>s.chord.notes.length!==current.notes.length||!s.chord.notes.every((n,i)=>n===current.notes[i]));suggestions.sort((a,b)=>b.score-a.score);return suggestions.map(s=>{let seq=[from,s.item,to].map(i=>chord(key,minor,i));return {...s,voices:voicings(seq,{...settings,loop:false,smooth:true})};});
}
